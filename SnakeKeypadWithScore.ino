...
byte rowPins[ROWS] = { 3, 2, 1, 0 }; // Rows 0..3 [R0,R1,R2,R3]
byte colPins[COLS] = { 4, 5, 6, 7 }; // Cols 0..3 [C0,C1,C2,C3]
...
void loop()
{
    ...
    String sScore(g_iScore);

    if (g_iScore > 0)
        lcd.write(sScore.c_str());
    else
        lcd.write("0 "); //clear the previous
}
